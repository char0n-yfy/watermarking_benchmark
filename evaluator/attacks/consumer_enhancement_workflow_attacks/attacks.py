...
from evaluator.attacks.content_preserve_workflow_attacks.helpers import (
    apply_filter_lut,
    auto_enhance,
    clahe,
    denoise,
...
def _deep_enhance(image: Image.Image, operation: str, params: Mapping[str, Any], context: AttackContext) -> tuple[Image.Image, dict[str, Any]]:
    ...
    try:
        if operation == "d1":
            from evaluator.attacks.content_preserve_workflow_attacks.deep_models import run_zero_dce_plus_plus
        ...
        if operation == "d5":
            from evaluator.attacks.content_preserve_workflow_attacks.deep_models import run_restormer_denoise
...
def _sr(image: Image.Image, params: Mapping[str, Any], context: AttackContext) -> tuple[Image.Image, dict[str, Any]]:
    ...
    try:
        if model_name in {"realesrgan_x2plus", "bsrgan_x2", "bsrgan_x2_rescaled"} and scale == 2:
            from evaluator.attacks.content_preserve_workflow_attacks.deep_models import run_rrdbnet_x2
        ...
        if model_name in {"realesrgan_x4plus", "bsrgan_x4"} and scale == 4:
            from evaluator.attacks.content_preserve_workflow_attacks.deep_models import run_rrdbnet_x4
        ...
        if model_name in {"swinir_x2", "swinir_x4"}:
            from evaluator.attacks.content_preserve_workflow_attacks.deep_models import run_swinir_classical_sr
