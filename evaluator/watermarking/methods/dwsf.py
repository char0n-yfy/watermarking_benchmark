...
class DWSFWatermark(BaseWatermark):
    ...
    def _encode_tensor(self, image, message):
        ...
        encoded_blocks = self._encoder_decoder.encoder(blocks_tensor, repeated_message)
    ...
    def embed_impl(
        ...
        self._tf.to_pil_image(((watermarked[0].detach().cpu() + 1.0) / 2.0).clamp(0, 1)).save(output_path)
        return {
