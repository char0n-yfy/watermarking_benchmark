...
from vine.src.model import make_1step_sched, my_vae_encoder_fwd, my_vae_decoder_fwd, download_url, sd_turbo_path


def _sd_variant():
    return os.environ.get("VINE_SD_TURBO_VARIANT", "fp16") or None


def _sd_dtype(device):
...
def initialize_unet(rank, return_lora_module_names=False):
    unet = UNet2DConditionModel.from_pretrained(sd_turbo_path(), subfolder="unet")
...
def initialize_unet_no_lora(path=None, device="cuda"):
    path = path or sd_turbo_path()
    unet = UNet2DConditionModel.from_pretrained(
        path,
        subfolder="unet",
        variant=_sd_variant(),
        torch_dtype=_sd_dtype(device),
    )
    unet = unet.float()
...
def initialize_vae_no_lora(path=None, device="cuda"):
    path = path or sd_turbo_path()
    ...
    vae.train()
    # add the skip connection convs
    vae.decoder.skip_conv_1 = torch.nn.Conv2d(512, 512, kernel_size=(3, 3), stride=(1, 1), padding=1, bias=True).to(device=device, dtype=torch.float32).requires_grad_(True)
    vae.decoder.skip_conv_2 = torch.nn.Conv2d(256, 512, kernel_size=(3, 3), stride=(1, 1), padding=1, bias=True).to(device=device, dtype=torch.float32).requires_grad_(True)
    vae.decoder.skip_conv_3 = torch.nn.Conv2d(128, 512, kernel_size=(3, 3), stride=(1, 1), padding=1, bias=True).to(device=device, dtype=torch.float32).requires_grad_(True)
    vae.decoder.skip_conv_4 = torch.nn.Conv2d(128, 256, kernel_size=(3, 3), stride=(1, 1), padding=1, bias=True).to(device=device, dtype=torch.float32).requires_grad_(True)
    torch.nn.init.constant_(vae.decoder.skip_conv_1.weight, 1e-5)
    torch.nn.init.constant_(vae.decoder.skip_conv_2.weight, 1e-5)
    torch.nn.init.constant_(vae.decoder.skip_conv_3.weight, 1e-5)
    torch.nn.init.constant_(vae.decoder.skip_conv_4.weight, 1e-5)
...
class VINE_Turbo(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(self, ckpt_path=None, device='cuda'):
        ...
        self.unet = initialize_unet_no_lora(device=device)
        self.vae_a2b = initialize_vae_no_lora(device=device)
    ...
    def forward(self, x, secret=None, timesteps=None):
        ...
        sec_dtype = next(self.sec_encoder.parameters()).dtype
        vae_enc_dtype = next(self.vae_enc.parameters()).dtype
        unet_dtype = next(self.unet.parameters()).dtype
        vae_dec_dtype = next(self.vae_dec.parameters()).dtype

        x_sec = self.sec_encoder(secret.to(dtype=sec_dtype), x.to(dtype=sec_dtype))
        x_enc = self.vae_enc(x_sec.to(dtype=vae_enc_dtype), direction="a2b")
        x_enc = x_enc.to(dtype=unet_dtype)
        fixed_emb = self.fixed_a2b_emb_base.to(device=x_enc.device, dtype=unet_dtype)
        model_pred = self.unet(x_enc, timesteps, encoder_hidden_states=fixed_emb,).sample
        x_out = torch.stack([self.sched.step(model_pred[i], timesteps[i], x_enc[i], return_dict=True).prev_sample for i in range(B)])
        x_out_decoded = self.vae_dec(x_out.to(dtype=vae_dec_dtype), direction="a2b").to(output_dtype)
