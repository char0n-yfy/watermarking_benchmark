...
def build_backbone(path, name):
    ...
    if path is not None:
        if path.startswith("http"):
            ...
            # For PyTorch >=2.6, `torch.load` defaults to weights_only=True, which
            # can break on older checkpoints that store numpy objects. We force
            # weights_only=False here (trusted checkpoint from this repo), and
            # fall back to the legacy signature if the argument is unsupported.
            try:
