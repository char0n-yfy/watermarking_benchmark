...
try:
    ...
    def _missing_augly(*args, **kwargs):
        raise RuntimeError("AugLy/libmagic is required for attack evaluation, but not for basic decode.")

    class _MissingAuglyFunctional:
        encoding_quality = staticmethod(_missing_augly)
        meme_format = staticmethod(_missing_augly)
        overlay_emoji = staticmethod(_missing_augly)
        overlay_onto_screenshot = staticmethod(_missing_augly)

    aug_functional = _MissingAuglyFunctional()
