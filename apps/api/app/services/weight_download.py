...
from app.services.watermark_weights import (
    iter_weight_files,
    weights_dir_name,
    weights_install_dir,
    weights_installed,
    weights_need_download,
)
...
class WeightDownloadService:
    def __init__(self, resources_root: Path, *, oss: ObjectStorageClient | None = None) -> None:
        self.resources_root = resources_root
        self.oss = oss
        self.cache_root = resources_root / "cache" / "watermark-weight-downloads"
        self.cache_root.mkdir(parents=True, exist_ok=True)
        self._jobs: dict[str, WeightDownloadJob] = {}
        self._lock = threading.Lock()

    def get_job(self, job_id: str) -> WeightDownloadJob:
        with self._lock:
            job = self._jobs.get(job_id)
        if job is None:
            raise KeyError(f"Unknown weight download job: {job_id}")
        return job

    def list_jobs(self, method: str | None = None) -> list[WeightDownloadJob]:
        with self._lock:
    ...
    def start_download(self, method: str) -> WeightDownloadJob:
        if not weights_need_download(method):
            raise ValueError(f"Watermark method does not use packaged weights: {method}")
    ...
    def _build_job_id(self, weights_dir: str) -> str:
        ...
        return f"{weights_dir}__weights__{stamp}_{suffix}"

    def _canonical_cache_archive(self, weights_dir: str) -> Path:
        return self.cache_root / f"{weights_dir}__weights.zip"

    def _set_progress(self, job: WeightDownloadJob, completed: int, total: int, *, message: str) -> None:
    ...
    def _run_download(self, job: WeightDownloadJob) -> None:
        install_dir = weights_install_dir(self.resources_root, job.method)
        if weights_installed(install_dir):
            self._mark_already_installed(job, install_dir)
            return
        ...
        if cached:
            job.message = "使用 cache 中的水印算法权重包"
            self._finalize_from_archive(job, cached, install_dir)
            return

        if not self.oss or not self.oss.enabled:
            raise FileNotFoundError(
                f"水印算法权重未安装且 OSS 未启用。请手动放入 {install_dir} 或配置 WM_BENCH_OSS_*。"
            )

        object_key = self.oss.watermark_weights_key(job.weights_dir)
        if not self.oss.exists(object_key):
            raise FileNotFoundError(
                f"OSS 上未找到权重包: {object_key}。"
                f"请上传 wmbench/weights/watermarking/{job.weights_dir}/weights.zip"
            )
    ...
    def _mark_already_installed(self, job: WeightDownloadJob, install_dir: Path) -> None:
        cached = self._find_cached_archive(job.weights_dir)
        ...
        self._set_progress(
            ...
            max(file_count, 1),
            max(file_count, 1),
            message=f"水印算法权重已安装（{install_dir.relative_to(self.resources_root)}），跳过重复下载",
        )

    def _find_cached_archive(self, weights_dir: str) -> Path | None:
        for cache_root in self._cache_roots():
            canonical = cache_root / f"{weights_dir}__weights.zip"
            if self._is_valid_cache_archive(canonical):
                return canonical
            for candidate in sorted(
                cache_root.glob(f"{weights_dir}__weights__*.zip"),
    ...
    def _finalize_from_archive(self, job: WeightDownloadJob, archive_path: Path, install_dir: Path) -> None:
        if weights_installed(install_dir):
            self._mark_already_installed(job, install_dir)
            return

        self._set_progress(job, 0, 1, message="解压到水印算法权重目录")
